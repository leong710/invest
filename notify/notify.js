...
// // 子技能--B
    ...
    function step4(lists_obj){
        return new Promise((resolve) => {
            // console.log('notifyLists...', lists_obj);
...
// 主技能
    ...
    async function load_init(action){
        ...
        try {
            mloading("show");                               // 啟用mLoading
            await load_fun(_type, 'bpm, true',     step1);  // 查詢bpm，並找出email
            await load_fun(_type, 'notify, true' , step2);  // step_2 先抓json，沒有then抓db(true/false 輸出json檔)，取得 highlight內容後鋪設內容
            // console.log(doc_lists);
            await step3(doc_lists, step4);
