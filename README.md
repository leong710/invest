...
事故訪談系統--職災PM需求
    ...
    ### 2026/01/05
        1. 修正：update_document()時增加 confirm_odd()的時機，避免於store_document()時已產生_odd所產生的更新錯誤。
        2. 修正：confirm_odd()增加$_content["s8_combo_01"] 公傷判定，避免'損工'但工傷=否的時候所產生的錯誤。
        3. 優化：列印時強化.tag和被disabled項目的灰色文字，強制轉成黑色，讓列印內容更清晰。
        4. 優化：列印時跳出溫馨提醒關閉[圖形背景]選項。
        5. 修正：錯字session => section
        6. 優化：當事人簽名欄增加(或其委任代理人)之字樣。
        7. PM要求增加欄位：
            7.1. 廠外交通事故：
                7.1.1. section_1 + 電話(個資問題：PM+開單人可以看到內容，其他人用password保護；列印時隱藏該項目)
                7.1.2. section_4 + 住院狀態(免住院、已出院+填出院日期、住院中+填區域+醫院名稱)，並連動section_4-1
                7.1.3. section_5 + 有無違反交通法規(有+填違規項目)，並連動section_8-1
            7.2. 人傷事故、設備/其他事故：
                7.2.1 section_2 + 責任區域負責單位、責任區域負責人

    ### 2026/01/22
        1. 修正：*** 住院狀態：NA/免住院 ==> NA-免住院  ,\/儲存符號造成對應錯誤。修正form_json/13ES100016-F002-V002b2.json/s4_combo_04