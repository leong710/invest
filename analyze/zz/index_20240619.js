    ...
    async function get_dccNo(fun, gain_obj){
        // return new Promise((resolve) => { 
            // console.log('fun: get_dccNo...', fun, gain_obj);
    ...
    async function caseList(fun, gain_obj){
        // return new Promise((resolve) => { 
            // // console.log('fun: caseList...', fun, gain_obj);
            // const promises = [];
            // Object(gain_obj).forEach((_doc)=>{
            //     let uuid = _doc['uuid'];
            //     promises.push(load_fun('document', uuid, gain_bigData));  // step_2 load_document(uuid);           // 20240501 -- 改由後端取得 _document內容
            // })
            // await Promise.all(promises);  // 確保所有文檔加載完成
            // resolve(); // 文件載入成功，resolve
            const promises = gain_obj.map(async (_doc) => {
                let uuid = _doc['uuid'];
                await load_fun('document', uuid, gain_bigData);
            });
            await Promise.all(promises);  // 確保所有文檔加載完成
        // });
    }
    ...
    async function analyze(gain_obj){
        // return new Promise((resolve) => { 
            ...
            Object.keys(gain_obj).forEach((doc_key)=>{
                // console.log(doc_key, gain_obj[doc_key]['label'], gain_obj[doc_key]['value']);
